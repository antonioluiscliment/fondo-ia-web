...
// Descarga los cierres de los 30 tickers (incluyendo la cotización
...
export async function obtenerDatosAlineados(yahooFinance, diasVentana = DIAS) {
  const porTicker = {};
...
// veces en cartera (ver contadorApariciones), de entre los 30. Un
// ticker que nunca apareció en el periodo cuenta como 0. Desempate:
// orden alfabético del ticker, igual que en seleccionarTop.
export function elegirTopPorVeces(contadorApariciones, nComponentes) {
...
// Rentabilidad del propio índice (Dow Jones, ^DJI) entre dos fechas,
// para poder comparar el modelo con "comprar y mantener el índice".
export async function obtenerRentabilidadIndice(yahooFinance, fechaInicioISO, fechaFinISO) {
  ...
  const resultado = await yahooFinance.chart("^DJI", {
    period1: desde,
    period2: hasta,
    interval: "1d",
  });
...
// Incremento porcentual diario del propio índice (Dow Jones, ^DJI)
// para cada fecha del array `fechas` (cierre de hoy / cierre de ayer
// − 1, en puntos porcentuales), para poder mostrarlo junto al
// beneficio de la cartera seleccionada cada día. Se descarga con
// margen de sobra por delante y por detrás para poder alinear por
// fecha exacta y disponer siempre de la sesión anterior a la primera
// fecha pedida. Devuelve también los cierres ya descargados (`cierres`)
// para que quien llame pueda calcular además la rentabilidad total del
// índice en el periodo sin tener que hacer una segunda descarga por
// separado (menos llamadas a Yahoo Finance, más margen frente a
// límites de tasa de la API).
export async function obtenerIncrementosIndice(yahooFinance, fechas) {
...
export function calcularPuntuacionesSesion(fechas, datosPorTicker, numeroSesion) {
  ...
  const puntuaciones = TICKERS.map((ticker) => {
...
export function calcularSeleccionCompleta(
  ...
  for (let t = SESIONES_PUNTUACION; t < fechas.length; t++) {
    const scoresDelDia = TICKERS.map((ticker) => {
