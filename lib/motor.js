...
export async function obtenerDatosAlineados(yahooFinance, diasVentana = DIAS, tickers) {
  ...
  // con datos — si se incluyera aquí alguno fallido (con cero fechas),
  // la intersección se quedaría vacía para todos, no solo para él.
  let fechasComunes = null;
  for (const ticker of tickersConDatos) {
...
export function calcularSeleccionCompleta(
  ...
  // réplica de índice, pages/api/replicaIndice.js) — el primer día de
  // cada serie ya viene garantizado con dato real, por el filtro de
  // arriba.
  for (const ticker of Object.keys(datosPorTicker)) {
    ...
    datosPorTicker[ticker] = serie.map((dia) => {
