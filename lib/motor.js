...
// Extrae la magnitud relevante de una entrada {fecha, cierre, volumen}
// según el criterio elegido. "flujo" es precio × volumen (dinero
// contratado); si falta cualquiera de los dos, se considera sin dato.
function valorMagnitud(entry, criterio) {
  if (!entry) return undefined;
  if (criterio === "volumen") return entry.volumen;
  if (criterio === "flujo") {
    if (entry.cierre === null || entry.cierre === undefined || entry.volumen === null || entry.volumen === undefined) {
      return undefined;
    }
    return entry.cierre * entry.volumen;
  }
  return entry.cierre; // precio
}

// Para volumen y flujo, el "índice de referencia" no viene de fuera
// (no tiene sentido pedir a Yahoo el volumen de contratación de
// ^IBEX, por ejemplo): se reconstruye agregando, sesión a sesión, la
// magnitud de todos los componentes disponibles ese día (suma
// simple, sin ponderar). Los componentes sin dato ese día
// simplemente no suman; si NINGÚN componente tiene dato, la sesión
// queda como null (arrastre de huecos ya gestionado aguas arriba en
// obtenerDatosAlineados).
function construirSerieIndiceAgregada(fechas, datosPorTicker, criterio) {
  const tickers = Object.keys(datosPorTicker);
  return fechas.map((_, i) => {
    let suma = 0;
    let algunValor = false;
    for (const ticker of tickers) {
      const valor = valorMagnitud(datosPorTicker[ticker][i], criterio);
      if (valor !== null && valor !== undefined) {
        suma += valor;
        algunValor = true;
      }
    }
    return algunValor ? suma : null;
  });
}
...
export function calcularReversionMedia(
  ...
  // Serie del "índice" comparada en ratioIndice: para precio es la
  // serie externa ya alineada (^IBEX, ^DJI...); para volumen y flujo
  // se reconstruye agregando los propios componentes.
  const serieIndice = criterio === "precio" ? cierresIndiceAlineados : construirSerieIndiceAgregada(fechas, datosPorTicker, criterio);

  function ratioTicker(ticker, inicio, fin) {
    const serie = datosPorTicker[ticker];
    const p0 = valorMagnitud(serie[inicio], criterio);
    const pt = valorMagnitud(serie[fin], criterio);
    if (p0 === null || p0 === undefined || p0 === 0 || pt === null || pt === undefined) return null;
    return pt / p0;
  }
  function ratioIndice(inicio, fin) {
    const i0 = serieIndice[inicio];
    const it = serieIndice[fin];
    if (i0 === null || i0 === undefined || i0 === 0 || it === null || it === undefined) return null;
    return it / i0;
  }

  // Ranking de mejor a peor (Pt/P0 - It/I0) en una ventana dada;
  // devuelve los "nPeores" seleccionados tras aplicar "nExclusion".
  // Con modo "peores" se cogen desde el final de la lista (peor
  // extremo); con modo "mejores" desde el principio (mejor extremo).
  // Se reutiliza tanto para cada ciclo cerrado como para la
  // formación "pendiente" de más abajo.
  function seleccionarPeores(inicio, fin) {
    const ratioIndiceVentana = ratioIndice(inicio, fin);
    const scores = tickers
      .map((ticker) => {
        const ratioAccion = ratioTicker(ticker, inicio, fin);
        if (ratioAccion === null || ratioIndiceVentana === null) return null;
        return { ticker, score: ratioAccion - ratioIndiceVentana };
      })
      .filter((s) => s !== null)
      .sort((a, b) => b.score - a.score || a.ticker.localeCompare(b.ticker));

    const n = scores.length;
    if (nPeores + nExclusion > n) return null;
    if (modo === "mejores") {
      return scores.slice(nExclusion, nExclusion + nPeores);
    }
    return scores.slice(n - nPeores - nExclusion, n - nExclusion);
  }

  const resultados = ciclos.map((ciclo, idx) => {
    const seleccionados = seleccionarPeores(ciclo.inicioFormacion, ciclo.finFormacion);
    if (!seleccionados) {
      return {
        ciclo: idx + 1,
  ...
  if (ciclos.length > 0) {
    ...
    const seleccionadosPendiente = seleccionarPeores(ultimoCiclo.inicioTest, ultimoCiclo.finTest);
    if (seleccionadosPendiente) {
      pendiente = {
        fechaInicioFormacion: fechas[ultimoCiclo.inicioTest],
        fechaFinFormacion: fechas[ultimoCiclo.finTest],
        valores: seleccionadosPendiente.map((s) => ({
          ticker: s.ticker,
          puntuacionFormacion: Number((s.score * 100).toFixed(3)),
        })),
      };
  ...
  return { ciclos: resultados, nCiclos: resultados.length, pendiente };
}
