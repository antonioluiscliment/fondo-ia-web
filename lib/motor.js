...
{ ventanaFormacion, ventanaTest = ventanaFormacion, solapado = false, nPeores = 3, nExclusion = 0, modo = "peores" } = {}
) {
  ...
  // (peor al final de la lista, como en el ejemplo acordado del Ibex
  // 35). Se reutiliza tanto para cada ciclo cerrado como para la
  // formación "pendiente" de más abajo.
  // La lista queda ordenada de mejor a peor (score descendente). Con
  // modo "peores" se cogen desde el final (igual que antes); con
  // modo "mejores" se cogen desde el principio, aplicando la misma
  // idea de "nExclusion" pero sobre el extremo contrario (excluye
  // los primeros N antes de coger la selección).
  ...
  // Ciclo pendiente: el último test ya calculado dobla como la
  // formación del ciclo siguiente (mismo esquema telescópico que
  // entre cualquier par de ciclos), pero ese ciclo siguiente aún no
  // tiene test porque haría falta conocer sesiones futuras. Se
  // muestra solo la selección resultante (los peores de esa
  // formación), sin ninguna rentabilidad de test todavía.
  let pendiente = null;
  if (ciclos.length > 0) {
