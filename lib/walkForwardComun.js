...
function ejecutarWalkForward(tickers, candidatosPorTicker, totalSesiones, paso, ajustarModelo, aplicarModelo) {
  ...
  // El bucle de entrenamiento se detiene "paso" sesiones antes de
  // hoy: el tramo más reciente todavía no tiene un resultado conocido
  // con el que entrenar (no se puede saber la rentabilidad futura de
  // algo que aún no ha pasado). La recomendación de HOY, más abajo,
  // se calcula aparte, con el modelo ya calibrado con todo lo
  // anterior — sin necesidad de conocer su resultado futuro.
  const finEntrenamiento = tHoy - paso;
  for (let t = VENTANA_WF; t <= finEntrenamiento; t += paso) {
    // 1) Puntuar HOY (de este paso) con el modelo actual (antes de
    // reajustarlo con el resultado de este mismo paso) — así la
    // recomendación de este paso usa solo información disponible
    // ANTES de conocer su resultado, nunca después.
    if (modeloActual !== null) {
      ...
      if (ranking.length > 0) {
        ...
        // todavía (eligió solo con datos hasta t, como corresponde).
        // Sirve para medir el historial real de cada modelo, no solo
        // su recomendación de hoy (que todavía no tiene resultado).
        ...
        historicoPasos.push({ t, ranking, top, retornoTopMedio, retornoBaseMedio });
      }
    }
  ...
  // Recomendación final: variables de HOY (el último día real, tHoy)
  ...
  // Historial real de rentabilidad: la media, a lo largo de TODOS los
  // pasos históricos, de "cómo le fue de verdad a lo que el modelo
  // recomendó en su momento" frente a "cómo le fue de media a
  // cualquiera de los candidatos ese día" — la comparación que
  // demuestra si el modelo aporta algo, en vez de una sola muestra de
  // hoy (que además todavía no tiene ningún resultado que mirar).
  const conRetorno = historicoPasos.filter((p) => p.retornoTopMedio !== null && p.retornoBaseMedio !== null);
  const media = (arr) => (arr.length > 0 ? Number((arr.reduce((a, b) => a + b, 0) / arr.length).toFixed(4)) : null);
  const retornoTopHistoricoMedio = media(conRetorno.map((p) => p.retornoTopMedio));
  const retornoBaseHistoricoMedio = media(conRetorno.map((p) => p.retornoBaseMedio));
  const pasosSuperaBase = conRetorno.filter((p) => p.retornoTopMedio > p.retornoBaseMedio).length;
  const tasaSuperaBase = conRetorno.length > 0 ? Number(((pasosSuperaBase / conRetorno.length) * 100).toFixed(1)) : null;
