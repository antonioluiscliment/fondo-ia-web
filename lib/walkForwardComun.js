...
// menos pasos comparables entre los dos modelos), no una limitación
// oculta: se avisa explícitamente en el resultado cuando se aplica.
//
// PASOS DISTINTOS PARA CADA MODELO: ridge se reajusta cada sesión
// (barato, solución matemática directa); la red, cada varias
// sesiones (entrenarla necesita iterar, así que reajustarla en cada
// sesión sería demasiado caro). Esto significa que solo hay puntos
// comparables entre los dos modelos en las sesiones de la red — con
// menos sesiones totales, también hay menos puntos comparables, y la
// fiabilidad de esa comparación se resiente más todavía.
...
export const TOTAL_SESIONES_WF_NORMAL = 70;
export const TOTAL_SESIONES_WF_REDUCIDO = 50;
...
// Compara las dos series de pasos SOLO en los puntos donde ambas
// tienen recomendación en la MISMA sesión (los pasos de la red son
// siempre un subconjunto de los pasos de ridge, dado que ambos
// empiezan en la misma sesión y el paso de la red es múltiplo del de
// ridge — por eso se recorren los de la red y se busca el de ridge en
// la misma sesión, no al revés).
