...
export const T = {
  es: {
    ...
    mejorFundamentalTitulo: "Selección por mejor fundamental",
    mejorFundamentalDesc: "Elige los componentes mejor clasificados HOY según el criterio fundamental elegido (Yahoo no da un histórico diario de estos datos, así que la clasificación no varía a lo largo de la ventana), y calcula sin ningún rebalanceo qué nº de componentes (entre 3 y 6) y qué pesos habrían dado la mayor rentabilidad acumulada en la ventana de backtest, respetando el tope de diversificación. PER, PER futuro y precio/valor contable se clasifican de menor a mayor (solo valores positivos); EPS, EPS futuro y rentabilidad por dividendo, de mayor a menor.",
  ...
  en: {
    ...
    mejorFundamentalTitulo: "Best fundamental selection",
    mejorFundamentalDesc: "Picks the best-ranked components TODAY according to the chosen fundamental criterion (Yahoo doesn't provide a daily history of this data, so the ranking doesn't change across the window), and works out, with no rebalancing at all, which number of components (between 3 and 6) and which weights would have given the highest cumulative return over the backtest window, within the diversification cap. P/E, forward P/E and price/book are ranked from lowest to highest (positive values only); EPS, forward EPS and dividend yield, from highest to lowest.",
