...
// ---------- PDF a partir de HTML libre (no una tabla) ----------
//
// Para contenido como "Especificaciones", que no es una tabla de
// datos sino texto con formato (viene de un .docx convertido a HTML
// con mammoth) — se extrae el texto plano, conservando los saltos de
// párrafo, y se pagina como texto corrido, con salto de página
// automático cuando no cabe más en la hoja actual.

function extraerTextoDeHtml(html) {
  if (typeof document === "undefined") return html.replace(/<[^>]+>/g, " ");
  const contenedor = document.createElement("div");
  contenedor.innerHTML = html;
  // Inserta un salto de línea antes de cada elemento de bloque, para
  // no perder la estructura de párrafos al pasar a texto plano.
  contenedor.querySelectorAll("p, br, h1, h2, h3, h4, li, tr").forEach((el) => {
    el.insertAdjacentText("beforebegin", "\n");
  });
  return contenedor.textContent || contenedor.innerText || "";
}

function construirDocumentoHtmlPdf({ titulo, html }) {
  const doc = new jsPDF();
  const anchoUtil = doc.internal.pageSize.getWidth() - 28;
  const altoPagina = doc.internal.pageSize.getHeight();
  ...
  const parrafos = extraerTextoDeHtml(html)
    .split(/\n+/)
    .map((p) => p.trim())
    .filter(Boolean);
...
// opciones: { titulo, html, nombreArchivo }
export function descargarHtmlPdf(opciones) {
  const doc = construirDocumentoHtmlPdf(opciones);
  doc.save(opciones.nombreArchivo);
}

export async function compartirHtmlPdf(opciones) {
  const doc = construirDocumentoHtmlPdf(opciones);
  const blob = doc.output("blob");
  const file = new File([blob], opciones.nombreArchivo, { type: "application/pdf" });
  try {
